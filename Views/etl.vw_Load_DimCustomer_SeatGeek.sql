...
CREATE VIEW [etl].[vw_Load_DimCustomer_SeatGeek] AS ( --Change "Source" to the correct source

------------------------------------------------------------------------------- 
-- Modification History -- 
-- 2018-07-23 Keegan Schmitt
-- Changed Fullname to be mapped from dcAddress3 becuse that is where it is coming in from
------------------------------------------------------------------------------- 
...
            , CAST((CASE WHEN ISNULL(c.dcFirstName, '') = '' AND ISNULL(cef.dcefStringField2, '') <> '' THEN c.dcLastName
				ELSE c.dcWorkPlace END) AS NVARCHAR (50)) AS [CompanyName] 
            , CASE WHEN ISNULL(c.dcFirstName, '') = '' AND ISNULL(cef.dcefStringField2, '') <> '' THEN NULL
				ELSE c.dcPreferedName END AS [SalutationName]
            , NULL AS [DonorMailName]
            , NULL AS [DonorFormalName]
            , TRY_CAST(CONVERT(VARCHAR(30),c.dcBirthday, 102) AS DATE) AS [Birthday] -- field is sensitive see writup in sharepoint for an example of how complicated it can get
            , NULL AS [Gender] 
            , 0 [MergedRecordFlag]
            , NULL [MergedIntoSSID]
 
...
            , CASE WHEN ISNULL(c.dcFirstName, '') = '' 
AND c.dcaddressline3 LIKE '%,%'
--AND ISNULL(cef.dcefStringField2, '') <> '' 
THEN LTRIM(SUBSTRING(c.dcAddressLine3, CHARINDEX(',', c.dcAddressLine3) + 1, LEN(c.dcAddressLine3)))
				ELSE c.dcFirstName END AS [FirstName]
            , CASE WHEN ISNULL(c.dcFirstName, '') = '' AND ISNULL(cef.dcefStringField2, '') <> '' THEN NULL
				ELSE c.dcMiddleName END AS [MiddleName]
            , CASE WHEN ISNULL(c.dcFirstName, '') = '' 
--AND ISNULL(cef.dcefStringField2, '') <> '' 
THEN SUBSTRING(c.dcAddressLine3, 0, charindex(',', c.dcAddressLine3, 0))
				ELSE c.dcLastName END AS [LastName]
            , CASE WHEN ISNULL(c.dcFirstName, '') = '' AND ISNULL(cef.dcefStringField2, '') <> '' THEN NULL
				ELSE c.dcSuffix END AS [Suffix] 
			, CASE WHEN ISNULL(c.dcAddressLine3, '') = '' AND c.dcaddressline3 LIKE '%,%'
			--AND ISNULL(cef.dcefStringField2, '') <> '' 
			THEN NULL
				ELSE c.dcAddressLine3 END AS [Fullname]   --  Changed to map in fullName from Address3 field, which is apparently how it is coming in
            --, c.name_title as [Title]
 
            /*AddressPrimary*/
            , CASE WHEN ISNULL(c.dcAddressLine2, '') <> '' THEN CONCAT(c.dcAddressLine1, ' ', c.dcAddressLine2)
				ELSE c.dcAddressLine1 END AS [AddressPrimaryStreet] --CASE WHEN dc.DimCustomerId IS NULL THEN  NULLIF(ISNULL(address1_line1,'') + ' ' + ISNULL(address1_line2,''),' ') ELSE address1_line1 END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
            , NULL AS [AddressPrimarySuite] -- ALWAYS LEAVE THIS NULL!!
			, c.dcCityGlobalName AS [AddressPrimaryCity] 
            , states.dsName AS [AddressPrimaryState] 
            , c.dcZipCode AS [AddressPrimaryZip] 
            , NULL AS [AddressPrimaryCounty]
            , country.dcnName  AS [AddressPrimaryCountry] 
             
...
 
            /*Phone*/
            , LEFT(c.dcHomePhone,25) AS [PhonePrimary]
            , LEFT(c.dcHomePhone,25) AS [PhoneHome]
            , LEFT(c.dcMobilePhone,25) AS [PhoneCell]
            , LEFT(c.dcBusinessPhone,25) AS [PhoneBusiness]
            , LEFT(c.dcFax,25) AS [PhoneFax]
            , LEFT(NULL,25) AS [PhoneOther]
 
            /*Email*/
            , CAST(c.dcEmail AS NVARCHAR(256)) AS [EmailPrimary]
...
            , 'SG-' + CAST(c.dcClientGUID AS NVARCHAR(100))  AS [ExtAttribute34] --nvarchar(max)
            , NULL AS [ExtAttribute35] --nvarchar(max)
 
            /*Source Created and Updated*/
            , NULL [SSCreatedBy] --createdby
            , NULL [SSUpdatedBy] --lastmodifiedbyid
            , c.dcJoinDate [SSCreatedDate] --createddate
            , c.dcLastUpdateDateTime [SSUpdatedDate] --lastmodifiedbyid
            , GETDATE () AS createddate 
            , GETDATE () AS updateddate
            , c.dcClientCode AS [AccountId]
            , CASE WHEN ISNULL(c.dcFirstName, '') = '' AND ISNULL(cef.dcefStringField2, '') <> '' THEN 1
				ELSE 0 END AS IsBusiness
			, NULL AS IsDeleted
            , CASE WHEN ISNULL(c.dcClientCode,'') = '' THEN NULL ELSE 'TM-' + c.dcClientCode END  AS customer_matchkey
...
        FROM ods.SGDW_DimClients c (NOLOCK)
		...
		--LEFT JOIN ods.SGDW_DimClientTypeDetails ctd (NOLOCK)
		--	ON c.dcClientGUID = ctd.dctdClientGuid
		--LEFT JOIN ods.SGDW_DimClientTypes ct (NOLOCK)
		--	ON ctd.dctdTypeGuid = ct.dctGuid
        WHERE 1=1
        --AND COALESCE(c.dcLastUpdateDateTime, c.dcJoinDate) >= (GETDATE() -3) --This needs to be uncommented out after the first load, update the date fields
    ) a
