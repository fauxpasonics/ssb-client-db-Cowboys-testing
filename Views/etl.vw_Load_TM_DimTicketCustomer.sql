...
CREATE VIEW [etl].[vw_Load_TM_DimTicketCustomer] AS (
	...
	LEFT OUTER JOIN (
		SELECT cr.acct_id, MAX(dr.DimRepId) DimRepId
		FROM ods.TM_CustRep (NOLOCK) cr
		INNER JOIN etl.vw_DimRep dr ON cr.acct_rep_id = dr.ETL__SSID_TM_acct_id
		WHERE cr.acct_rep_type = 'S'
		GROUP BY cr.acct_id
	) dr ON c.acct_id = dr.acct_id
	WHERE c.Primary_code = 'Primary'
	AND c.UpdateDate > GETDATE() - 2
)
