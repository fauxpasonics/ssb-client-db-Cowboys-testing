...
CREATE VIEW [etl].[vw_Load_DimCustomer_Personicx] AS ( --Change "Source" to the correct source
 
    SELECT *
    /*Name*/
    , HASHBYTES('sha2_256',
...
 
            /*AddressPrimary*/
            , CASE WHEN ISNULL([AddressPrimarySuite], '') <> '' AND ISNULL([AddressPrimaryStreet], '') <> ISNULL([AddressPrimarySuite], '') THEN CONCAT([AddressPrimaryStreet], ' ', [AddressPrimarySuite])
				ELSE [AddressPrimaryStreet] END AS [AddressPrimaryStreet] --CASE WHEN dc.DimCustomerId IS NULL THEN  NULLIF(ISNULL(address1_line1,'') + ' ' + ISNULL(address1_line2,''),' ') ELSE address1_line1 END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
            , NULL AS [AddressPrimarySuite] -- ALWAYS LEAVE THIS NULL!!
			, AddressPrimaryCity AS [AddressPrimaryCity] 
            , AddressPrimaryState AS [AddressPrimaryState] 
            , AddressPrimaryZip AS [AddressPrimaryZip] 
            , AddressPrimaryCounty AS [AddressPrimaryCounty]
...
 
            /*Phone*/
            , LEFT(PhonePrimary,25) AS [PhonePrimary]
...
 
            /*Email*/
            , CAST(EmailPrimary AS NVARCHAR(256)) AS [EmailPrimary]
...
            , CAST(ETL__CreatedDate AS DATETIME2) [SSCreatedDate] --createddate
            , CAST(ETL__UpdatedDate AS DATETIME2) [SSUpdatedDate] --lastmodifiedbyid
...
--		SELECT *           
        FROM dbo.Personicx_Appends (NOLOCK)
        WHERE 1=1
        AND (ETL__CreatedDate >= DATEADD(DAY,-3,GETDATE()) OR ETL__UpdatedDate >= DATEADD(DAY,-3,GETDATE())
)
 ) a
 
)
