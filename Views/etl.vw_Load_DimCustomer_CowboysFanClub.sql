...
CREATE VIEW [etl].[vw_Load_DimCustomer_CowboysFanClub] AS 
...
, HASHBYTES('sha2_256', ISNULL(RTRIM(ExtAttribute31),'DBNULL_TEXT') 
	...
	--base set
        SELECT
            ...
            , 'Dynamics_FanClub' AS [SourceSystem] --Put source name such as TM or CRM_Contact
            ...
            /*AddressPrimary*/
            , NULLIF(ISNULL(address1_line1,'') + ' ' + ISNULL(address1_line2,''),' ') AS [AddressPrimaryStreet] --CASE WHEN dc.DimCustomerId IS NULL THEN  NULLIF(ISNULL(address1_line1,'') + ' ' + ISNULL(address1_line2,''),' ') ELSE address1_line1 END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
...
--		select top 100 *          
        FROM Cowboys_reporting.prodcopy.Contact_FanClub z WITH (NOLOCK) 
