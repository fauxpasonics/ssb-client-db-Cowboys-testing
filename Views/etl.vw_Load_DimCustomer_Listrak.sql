...
CREATE VIEW [etl].[vw_Load_DimCustomer_Listrak] AS ( --Change "Source" to the correct source
 
    SELECT *
    /*Name*/
    , HASHBYTES('sha2_256',
...
            , 'Listrak' AS [SourceSystem] --Put source name such as TM or CRM_Contact
            , NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules
 
            /*Standard Attributes*/
            , CAST(CONCAT(c.ListID,':',c.EmailAddress) AS NVARCHAR(50)) [SSID] -- Required
            , CAST(c.SubscriptionState AS NVARCHAR (50)) AS [CustomerType]
...
            , TRY_CAST(CONVERT(VARCHAR(30),NULL, 102) AS DATE) AS [Birthday] -- field is sensitive see writup in sharepoint for an example of how complicated it can get
            , c.Gender AS [Gender] 
            , 0 [MergedRecordFlag]
            , NULL [MergedIntoSSID]
 
...
 
            /*AddressPrimary*/
            , NULL AS [AddressPrimaryStreet] --CASE WHEN dc.DimCustomerId IS NULL THEN  NULLIF(ISNULL(address1_line1,'') + ' ' + ISNULL(address1_line2,''),' ') ELSE address1_line1 END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
            , NULL AS [AddressPrimarySuite] -- ALWAYS LEAVE THIS NULL!!
			, NULL AS [AddressPrimaryCity] 
            , NULL AS [AddressPrimaryState] 
            , c.ZipCode AS [AddressPrimaryZip] 
...
 
            /*Email*/
            , CAST(c.EmailAddress AS NVARCHAR(256)) AS [EmailPrimary]
            , CAST(NULL AS NVARCHAR(256)) AS [EmailOne]
            , CAST(NULL AS NVARCHAR(256)) AS [EmailTwo]
 
            /*Extended Attributes*/
            , CASE WHEN c.ATTStadiumEventsAndTicketOffers = 'on' THEN 'ATTStadiumEventsAndTicketOffers' ELSE NULL END AS [ExtAttribute1] --nvarchar(100)
            , CASE WHEN c.DCEventsAndTicketOffers = 'on' THEN 'DCEventsAndTicketOffers' ELSE NULL END AS [ExtAttribute2] --nvarchar(100)
            , CASE WHEN c.DCFanPlaybook = 'on' THEN 'DCFanPlaybook' ELSE NULL END AS [ExtAttribute3] --nvarchar(100)
            , CASE WHEN c.DCNewsletter = 'on' THEN 'DCNewsletter' ELSE NULL END AS [ExtAttribute4] --nvarchar(100)
            , CASE WHEN c.DCProShopOffers = 'on' THEN 'DCProShopOffers' ELSE NULL END AS [ExtAttribute5] --nvarchar(100)
            , CASE WHEN c.FordCenterEventsAndTicketOffers = 'on' THEN 'FordCenterEventsAndTicketOffers' ELSE NULL END AS [ExtAttribute6] --nvarchar(100)
...
            , c.SubscribeMethod [SSCreatedBy] --createdby
            , NULL [SSUpdatedBy] --lastmodifiedbyid
            , c.SubscribeDate [SSCreatedDate] --createddate
            , c.ETL__UpdatedDate [SSUpdatedDate] --lastmodifiedbyid
...
        --SELECT *   
		--select count(*) 
		FROM ods.Listrak_SubscribedContacts c   
        --FROM etl.vw_Listrak_ContactPivot c
        WHERE 1=1
        AND c.ETL__UpdatedDate >= (GETDATE() -3) --This needs to be uncommented out after the first load, update the date fields
    ) a
 
)
