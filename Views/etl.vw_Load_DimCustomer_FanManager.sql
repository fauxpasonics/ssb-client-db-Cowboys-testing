...
CREATE VIEW [etl].[vw_Load_DimCustomer_FanManager] AS ( --Change "Source" to the correct source
 
    SELECT *
    /*Name*/
    , HASHBYTES('sha2_256',
...
            , CAST(CASE WHEN r.BusinessName = '' THEN NULL ELSE r.BusinessName END AS NVARCHAR (50)) AS [CompanyName] 
...
            /*Name*/
            , NULL AS [Prefix]
            , CASE WHEN r.FirstName = '' THEN NULL ELSE r.FirstName END AS [FirstName]
            , NULL AS [MiddleName]
            , CASE WHEN r.LastName = '' THEN NULL ELSE r.LastName END AS [LastName]
            , NULL AS [Suffix]
			, NULL AS [Fullname]
            --, NULL as [Title]
 
            /*AddressPrimary*/
            , CASE WHEN r.Street1 <> '' AND r.Street2 <> '' AND r.Street1 <> r.Street2
					THEN CONCAT(r.Street1, ' ', r.Street2)
				WHEN r.Street1 <> '' AND r.Street2 = '' THEN r.Street1
				ELSE NULL END AS [AddressPrimaryStreet] --CASE WHEN dc.DimCustomerId IS NULL THEN  NULLIF(ISNULL(address1_line1,'') + ' ' + ISNULL(address1_line2,''),' ') ELSE address1_line1 END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
            , NULL AS [AddressPrimarySuite] -- ALWAYS LEAVE THIS NULL!!
			, CASE WHEN r.City = '' THEN NULL ELSE r.City END AS [AddressPrimaryCity] 
            , CASE WHEN r.[State] = '' THEN NULL ELSE r.[State] END AS [AddressPrimaryState] 
            , CASE WHEN r.ZipCode = '' THEN NULL ELSE r.ZipCode END AS [AddressPrimaryZip] 
            , NULL AS [AddressPrimaryCounty]
            , CASE WHEN r.Country = '' THEN NULL ELSE r.Country END AS [AddressPrimaryCountry] 
             
...
 
            /*Email*/
            , CAST(CASE WHEN r.Email = '' THEN NULL ELSE r.Email END AS NVARCHAR(256)) AS [EmailPrimary]
...
            , NULL AS [SSCreatedBy] --createdby
            , NULL AS [SSUpdatedBy] --lastmodifiedbyid
            , NULL AS [SSCreatedDate] --createddate
            , NULL AS [SSUpdatedDate] --lastmodifiedbyid
...
        --SELECT *           
        FROM dbo.FanManager_Contacts_Historical r (NOLOCK)
        WHERE 1=1
        --AND r.AdditionDate >= (GETDATE() -3) --This needs to be uncommented out after the first load, update the date fields
    ) a
 
)
