...
, HASHBYTES('sha2_256', ISNULL(RTRIM(ExtAttribute31),'DBNULL_TEXT') 
	...
	--base set
        SELECT
            'Cowboys_Reporting' AS [SourceDB] --Client Source Database such as Raiders or Raiders_Reporting
            ...
            --, new_ssbcrmsystem_recordtypename AS[ExtAttribute9] --nvarchar(100)
            ...
            , z.parentcustomerid AS [ExtAttribute31] --nvarchar(max)
            , NULL AS [ExtAttribute32] --nvarchar(max)
            , NULL AS [ExtAttribute33] --nvarchar(max)
            , CASE WHEN ISNULL(z.new_ssbcrmsystemSSIDWinner,'') = '' OR ISNULL(z.new_ssbcrmsystemSSIDWinnerSourceSystem,'') = '' THEN NULL
			WHEN z.new_ssbcrmsystemSSIDWinnerSourceSystem = 'SG' THEN 'SG-' + z.new_ssbcrmsystemSSIDWinner
			WHEN z.new_ssbcrmsystemSSIDWinnerSourceSystem = 'TM' THEN 'TM-' + z.new_ssbcrmsystemSSIDWinner
			ELSE NULL END
			AS [ExtAttribute34] --nvarchar(max)
            ...
            /*, CASE WHEN (SELECT value FROM [dbo].[fn_Split](str_number,', ') WHERE position = 1) IS NOT NULL
				THEN  (SELECT value FROM [dbo].[fn_Split](str_number,', ') WHERE position = 1) ELSE NULL
				END AS [AccountId]*/
			...
			/*, CASE WHEN z.new_SSBCRMSYSTEMSSIDWinner IS NULL OR z.new_SSBCRMSystemSSIDWinnerSourceSystem IS NULL THEN NULL
			   WHEN z.new_SSBCRMSystemSSIDWinnerSourceSystem = 'Listrak' THEN 'List-' + z.new_SSBCRMSYSTEMSSIDWinner
			...
			, (SELECT CASE WHEN (SELECT value FROM [dbo].[fn_Split](str_number,'|') WHERE position = 1) IS NOT NULL
    THEN 'TM-' + (SELECT RTRIM(value) FROM [dbo].[fn_Split](str_number,'|') WHERE position = 1)
    END) customer_matchkey 
    --        , CASE WHEN (SELECT value FROM [dbo].[fn_Split](str_number,', ') WHERE position = 1) IS NOT NULL
				--THEN 'TM-' + (SELECT value FROM [dbo].[fn_Split](str_number,', ') WHERE position = 1)
				--END customer_matchkey 
...
--		select top 100 *          
        FROM Cowboys_reporting.prodcopy.contact z WITH (NOLOCK) 
		LEFT JOIN dimcustomer dc ON CAST(z.contactid AS NVARCHAR(100)) = dc.ssid 
        WHERE (z.modifiedon >= DATEADD(DAY,-3,GETDATE())
		OR z.copyloaddate >= DATEADD(DAY,-3,GETDATE()))
		AND z.copyloaddate > '2018-07-14 16:43:29.953'
