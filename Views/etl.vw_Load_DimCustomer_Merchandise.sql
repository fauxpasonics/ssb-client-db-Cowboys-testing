...
CREATE VIEW [etl].[vw_Load_DimCustomer_Merchandise] AS ( --Change "Source" to the correct source
 
    SELECT *
    /*Name*/
    , HASHBYTES('sha2_256',
...
            , 'Merchandise' AS [SourceSystem] --Put source name such as TM or CRM_Contact
            , NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules
 
            /*Standard Attributes*/
            , CAST(mc.SSID AS NVARCHAR(50)) [SSID] -- Required
...
 
            /*AddressPrimary*/
            , COALESCE(mc.POBox, mc.StreetAddress) AS [AddressPrimaryStreet] --CASE WHEN dc.DimCustomerId IS NULL THEN  NULLIF(ISNULL(address1_line1,'') + ' ' + ISNULL(address1_line2,''),' ') ELSE address1_line1 END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
            , NULL AS [AddressPrimarySuite] -- ALWAYS LEAVE THIS NULL!!
			, mc.City AS [AddressPrimaryCity] 
            , mc.[State] AS [AddressPrimaryState] 
            , mc.PostalCode AS [AddressPrimaryZip] 
            , mc.County AS [AddressPrimaryCounty]
            , mc.Country AS [AddressPrimaryCountry] 
             
...
 
            /*Email*/
            , CAST(mc.Email AS NVARCHAR(256)) AS [EmailPrimary]
...
            , mc.CreatedDate [SSCreatedDate] --createddate
...
        --SELECT *           
        FROM etl.vw_Merch_Customers mc
        WHERE 1=1
        AND mc.CreatedDate >= (GETDATE() -3) --This needs to be uncommented out after the first load, update the date fields
    ) a
 
)
