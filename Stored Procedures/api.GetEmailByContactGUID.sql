...
SELECT DISTINCT
	  ...
	  , ms.Subject
	  , c.LinkUrl
	  , CASE WHEN BounceCount = 1 THEN 0 ELSE 1 END DeliveryResult
	  , 1-BounceCount DeliveryCount
	  , mca.SendDate
	  , mca.Opened
	  , o.OpenDate
	  , mca.ClickCount
	  , c.ClickDate
INTO #Emails
FROM    ods.Listrak_MessageContactActivity mca (NOLOCK)
		JOIN ods.Listrak_MessageSummary (NOLOCK) ms ON mca.MsgID = ms.MessageID
		LEFT JOIN ods.Listrak_Click c (NOLOCK) ON c.ContactID = mca.ContactID AND c.MessageID = ms.MessageID
		LEFT JOIN ods.Listrak_Open o (NOLOCK) ON o.ContactID = mca.ContactID AND o.MessageID = ms.MessageID
		JOIN dbo.vwDimCustomer_ModAcctId dc ON mca.ContactID = dc.ssid AND dc.SourceSystem = 'Listrak'
